...
const QSET_V8_CAMPAIGN_ID: &str = "campaign-2026-09-09-qset-v8";
...
pub type QsetV8WriterRetirementReceipt = QsetV6WriterRetirementReceipt;
...
impl RuntimeController {
    ...
    pub async fn prepare_qset_v8_retirement(
        ...
        if self.inner.settings.deploy.app_name != QSET_V8_APP_NAME
            || self.inner.settings.azure.storage_container_name != QSET_V8_RAW_CONTAINER
        {
            return Err(
                "qset-v8 writer retirement requires the exact app and raw container".to_owned(),
            );
        }
        self.shutdown().await?;
        self.qset_v8_retirement_receipt(qset_v8_image_digest()?, qset_v8_source_revision()?)
    }

    fn qset_v8_retirement_receipt(
        &self,
        image_digest: String,
        source_revision: String,
    ) -> Result<QsetV8WriterRetirementReceipt, String> {
        ...
        {
            return Err(
                "qset-v8 writer retirement recorder waterline is not fully durable".to_owned(),
            );
        }
        Ok(QsetV8WriterRetirementReceipt {
            schema: "polyedge.qset_v8_writer_retirement_receipt.v1",
            status: "prepared_for_retirement",
            retired_at: Utc::now(),
            campaign_id: QSET_V8_CAMPAIGN_ID,
            app_name: self.inner.settings.deploy.app_name.clone(),
            image_digest,
            source_revision,
            recorder_instance_id: metrics.recorder_instance_id.clone(),
...
fn qset_v8_image_digest() -> Result<String, String> {
    let image = std::env::var("POLYEDGE_QSET_V8_WRITER_IMAGE")
        .map_err(|_| "POLYEDGE_QSET_V8_WRITER_IMAGE is required for retirement".to_owned())?;
    qset_v8_image_digest_from(&image)
        .ok_or_else(|| "POLYEDGE_QSET_V8_WRITER_IMAGE must be pinned by SHA-256 digest".to_owned())
}
fn qset_v8_image_digest_from(image: &str) -> Option<String> {
    qset_v7_image_digest_from(image)
}
fn qset_v8_source_revision() -> Result<String, String> {
    let configured = std::env::var("POLYEDGE_QSET_V8_WRITER_GIT_SHA")
        .map_err(|_| "POLYEDGE_QSET_V8_WRITER_GIT_SHA is required for retirement".to_owned())?;
    qset_v8_source_revision_from(&configured, embedded_git_sha())
}
fn qset_v8_source_revision_from(
    configured: &str,
    embedded: Option<&str>,
) -> Result<String, String> {
    if !polyedge_config::is_full_git_sha(configured) {
        return Err("POLYEDGE_QSET_V8_WRITER_GIT_SHA must be an exact lowercase commit".to_owned());
    }
    if embedded.is_some_and(|embedded| embedded != configured) {
        return Err("qset-v8 frozen source revision does not match the running binary".to_owned());
    }
    Ok(configured.to_owned())
}
...
mod tests {
    ...
    #[test]
    fn qset_v8_receipt_requires_exact_frozen_identity() {
        let digest = format!("registry/polyedge@sha256:{}", "a".repeat(64));
        assert_eq!(
            qset_v8_image_digest_from(&digest),
            Some(format!("sha256:{}", "a".repeat(64)))
        );
        assert!(qset_v8_image_digest_from("registry/polyedge:latest").is_none());
        let revision = "a".repeat(40);
        assert_eq!(
            qset_v8_source_revision_from(&revision, Some(&revision)).unwrap(),
            revision
        );
        assert!(qset_v8_source_revision_from(&"A".repeat(40), None).is_err());
    }
