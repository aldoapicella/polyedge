...
@description('Public GHCR image pinned by immutable digest for qset-v4 processing.')
...
@description('Exact SHA-256 inventory for the read-only 2026-08-25 sealed raw day.')
...
var campaignId = 'campaign-2026-08-24-qset-v4'
var campaignStart = '2026-08-24'
var sealedDayOne = '2026-08-24'
var sealedDayTwo = '2026-08-25'
var rawContainerName = 'polyedge-shadow-qset-v4-events'
var researchContainerName = 'polyedge-research-qset-v4'
var controlContainerName = 'polyedge-qset-v4-control'
var eventIndexTableName = 'ShadowQsetV4EventIndex'
var chartSeriesTableName = 'ShadowQsetV4ChartSeries'
var marketCatalogTableName = 'ShadowQsetV4MarketCatalog'
...
var processorJobName = 'polyedge-shadow-qset-v4-neu-job'
...
var tags = {
  app: 'polyedge'
  managedBy: 'bicep'
  workload: 'profitability-shadow-qset-v4'
...
resource writerRawContributor 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  ...
  properties: {
    ...
    description: 'Only write or append qset-v4 preflight and campaign raw prefixes; the custom role has no delete, move, or container actions.'
  }
}
...
resource processorCampaignWriter 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  ...
  properties: {
    ...
    description: 'Only write or append qset-v4 campaign research and report prefixes; the custom role has no delete, move, or container actions.'
  }
}
          ...
          args: [
            ...
            '''
              validate_day() {
                date="$1"
                inventory="$2"
                validation="$(polyedge-rs seal-qset-v4-day --account "$AZURE_STORAGE_ACCOUNT_NAME" --client-id "$AZURE_CLIENT_ID" --date "$date" --source-freeze-blob "$EXECUTION_FREEZE_ARTIFACT_PATH" --source-freeze-sha256 "$EXECUTION_FREEZE_SHA256" --validate-only)"
                printf '%s
' "$validation" | jq -e --arg date "$date" --arg inventory "$inventory" --arg freeze "$EXECUTION_FREEZE_SHA256" --arg freeze_path "$EXECUTION_FREEZE_ARTIFACT_PATH" '
                  .schema == "polyedge.qset_v4_closed_day_validation.v1"
                  and .campaign_id == "campaign-2026-08-24-qset-v4"
                  and .date == $date
                  and .container == "polyedge-shadow-qset-v4-events"
                  and .blob_count == 1440
                  and .all_sealed == true
...
' "$hash" | grep -Eq '^sha256:[0-9a-f]{64}$' || exit 1
              ...
              case "$EXECUTION_FREEZE_ARTIFACT_PATH" in
                reports/research/shadow/campaigns/campaign-2026-08-24-qset-v4/control/code-freeze/source-[A-Za-z0-9._-]*.json) ;;
                *) exit 1 ;;
              esac
              validate_day '2026-08-24' "$SEALED_DAY_ONE_INVENTORY_SHA256"
              validate_day '2026-08-25' "$SEALED_DAY_TWO_INVENTORY_SHA256"
              exec /app/research/run_shadow_daily_v4.sh
            '''
          ]
          env: [
            ...
            { name: 'QSET_V4_CONTROL_CONTAINER_NAME', value: controlContainer.name }
            ...
            { name: 'SHADOW_CAMPAIGN_CONTRACT', value: 'research/configs/profitability_gate_v3_2026-08-24_qset_v4.yaml' }
            ...
            { name: 'SHADOW_EVIDENCE_VERSION', value: 'protocol-v3-qset-v4' }
