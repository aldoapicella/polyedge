...
freeze=$tmp/polyedge-qset-v8-source-freeze
sed "s#receipt_root=/srv/polyedge-ring/migration/qset-v8/source-freeze#receipt_root=$receipt_root#" "$root/bin/polyedge-qset-v8-source-freeze" >"$freeze"
chmod 0755 "$freeze"
...
jq -n --arg commit "$commit" --arg tree "$tree" --arg image "$image" '{
  ...
  evidence_version:"protocol-v3-qset-v8",
...
case "$*" in
  ...
  *"show HEAD:research/configs/campaign_freeze_2026-09-09_qset_v8.json")
...
! find "$tmp" -maxdepth 1 -type f \( -name 'tmp.*' -o -name '.qset-v8-freeze.*' \) | grep -q .
