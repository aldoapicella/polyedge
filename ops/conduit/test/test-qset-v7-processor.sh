...
preflight=$root/bin/polyedge-qset-v7-processor-preflight
handoff=$root/bin/polyedge-qset-v7-processor-handoff
env_file=$root/env/qset-v7-processor.env.example
service=$root/systemd/polyedge-qset-v7-processor.service
...
if env -i PATH="$PATH" "$preflight" >/dev/null 2>&1; then
  echo 'qset-v7 processor preflight accepted missing bindings' >&2
  exit 1
fi
grep -F '  qset-v7-processor)' "$runner" >/dev/null
grep -F 'cpus=4 memory=8g limit=16800' "$runner" >/dev/null
grep -F '/usr/local/libexec/polyedge-qset-v7-processor-preflight' "$runner" >/dev/null
grep -F 'credential=shadow-qset-v7-processor' "$runner" >/dev/null
...
preflight_line=$(grep -n '/usr/local/libexec/polyedge-qset-v7-processor-preflight' "$runner" | cut -d: -f1)
...
grep -F 'qset-v7-seal/2026-09-02.json' "$preflight" >/dev/null
grep -F 'qset-v7-seal/2026-09-03.json' "$preflight" >/dev/null
...
grep -F 'polyedge.qset_v7_source_freeze_upload_receipt.v2' "$preflight" >/dev/null
...
grep -Fx 'Requires=network-online.target polyedge-network.service polyedge-federated-token@shadow-qset-v7-processor.service' "$service" >/dev/null
grep -Fx 'ConditionPathExists=/etc/polyedge/ENABLE_QSET_V7_PROCESSOR_MANUAL' "$service" >/dev/null
grep -Fx 'ExecStartPre=/usr/bin/rm -f /etc/polyedge/ENABLE_QSET_V7_PROCESSOR_MANUAL' "$service" >/dev/null
...
! test -e "$root/systemd/polyedge-qset-v7-processor.timer"
...
jq -e '
  ...
  and ([.ociOnlyJobs[] | select(.name == "qset-v7-processor")] == [{
    ...
    requiredInputs:{sourceFreezeReceipt:"/srv/polyedge-ring/migration/qset-v7/source-freeze/source-<final-sha256>.json",sealedDayReceipts:["/srv/polyedge-ring/migration/qset-v7-seal/2026-09-02.json","/srv/polyedge-ring/migration/qset-v7-seal/2026-09-03.json"]},
