...
run "$repair" FAKE_PRIOR_SIGNER_IMAGE="$prior_signer_image" FAKE_REPAIR_PRE=1 POLYEDGE_GUARDED_RESTART_PRIOR_SIGNER_IMAGE="$prior_signer_image" POLYEDGE_GUARDED_RESTART_REPAIR_MODE=true POLYEDGE_GUARDED_RESTART_DEPLOY="$repair/bin/deploy" POLYEDGE_GUARDED_RESTART_QUADLET="$repair/quadlet" POLYEDGE_GUARDED_RESTART_ROLLBACK_DIR="$repair/rollback"
test "$(cat "$repair/state/producer-active")" = 1; grep -Fx "Image=$signer_image" "$repair/quadlet" >/dev/null
jq -e --arg prior "$prior_signer_image" --arg image "$signer_image" --arg revision "$signer_revision" '.status == "validated" and .startMode == "repair_rollout" and .signer.repairMode == true and .signer.priorImage == $prior and .signer.image == $image and .signer.revision == $revision and (.signer.rollbackCopy | endswith("fixture-polyedge-funded-signer.container")) and .producer.stoppedForRestart == true' "$repair/ring/activation/receipt.json" >/dev/null

repair_bad=$root/repair-bad; fixture "$repair_bad"; bind_repair_evidence "$repair_bad"
if run "$repair_bad" FAKE_PRIOR_SIGNER_IMAGE="$prior_signer_image" FAKE_REPAIR_PRE=1 FAKE_BAD_POST=1 POLYEDGE_GUARDED_RESTART_PRIOR_SIGNER_IMAGE="$prior_signer_image" POLYEDGE_GUARDED_RESTART_REPAIR_MODE=true POLYEDGE_GUARDED_RESTART_DEPLOY="$repair_bad/bin/deploy" POLYEDGE_GUARDED_RESTART_QUADLET="$repair_bad/quadlet" POLYEDGE_GUARDED_RESTART_ROLLBACK_DIR="$repair_bad/rollback"; then exit 1; fi
