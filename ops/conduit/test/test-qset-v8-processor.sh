...
preflight=$root/bin/polyedge-qset-v8-processor-preflight
handoff=$root/bin/polyedge-qset-v8-processor-handoff
env_file=$root/env/qset-v8-processor.env.example
service=$root/systemd/polyedge-qset-v8-processor.service
...
if env -i PATH="$PATH" "$preflight" >/dev/null 2>&1; then
  echo 'qset-v8 processor preflight accepted missing bindings' >&2
  exit 1
fi
grep -F '  qset-v8-processor)' "$runner" >/dev/null
grep -F 'cpus=4 memory=8g limit=16800' "$runner" >/dev/null
grep -F '/usr/local/libexec/polyedge-qset-v8-processor-preflight' "$runner" >/dev/null
grep -F 'credential=shadow-qset-v8-processor' "$runner" >/dev/null
...
grep -F '2026-09-09 --source-freeze-blob' "$runner" >/dev/null
grep -F '2026-09-10 --source-freeze-blob' "$runner" >/dev/null
grep -F '/app/research/run_shadow_daily_v8.sh' "$runner" >/dev/null
if awk '/^  qset-v8-processor\)/{copy=1} copy{print} copy && /^    ;;$/{exit}' "$runner" | grep -q 'qset-v7\|run_shadow_daily_v7'; then echo 'qset-v8 processor dispatches a v7 campaign path' >&2; exit 1; fi
mount_line=$(grep -n 'mountpoint -q "$ring"' "$runner" | cut -d: -f1)
preflight_line=$(grep -n '/usr/local/libexec/polyedge-qset-v8-processor-preflight' "$runner" | cut -d: -f1)
...
grep -F 'qset-v8-seal/2026-09-09.json' "$preflight" >/dev/null
grep -F 'qset-v8-seal/2026-09-10.json' "$preflight" >/dev/null
...
grep -F 'polyedge.qset_v8_source_freeze_upload_receipt.v2' "$preflight" >/dev/null
...
for exact in \
  ...
  'ALLOW_LIVE=false' \
  'AZURE_STORAGE_CONTAINER_NAME=polyedge-research-qset-v8' \
  'SHADOW_SOURCE_CONTAINER_NAME=polyedge-shadow-qset-v8-events' \
  'QSET_V8_CONTROL_CONTAINER_NAME=polyedge-qset-v8-control' \
...
grep -Fx 'Requires=network-online.target polyedge-network.service polyedge-federated-token@shadow-qset-v8-processor.service' "$service" >/dev/null
grep -Fx 'ConditionPathExists=/etc/polyedge/ENABLE_QSET_V8_PROCESSOR_MANUAL' "$service" >/dev/null
grep -Fx 'ExecStartPre=/usr/bin/rm -f /etc/polyedge/ENABLE_QSET_V8_PROCESSOR_MANUAL' "$service" >/dev/null
...
! test -e "$root/systemd/polyedge-qset-v8-processor.timer"

jq -e '
  .azureJobCount == (.jobs | length)
  and ([.ociOnlyJobs[] | select(.name == "qset-v8-processor")] == [{
    name:"qset-v8-processor",classification:"configured_manual_only_not_executed",
    ociUnit:"polyedge-qset-v8-processor.service",identityLane:"shadow-qset-v8-processor",
    ...
    requiredInputs:{sourceFreezeReceipt:"/srv/polyedge-ring/migration/qset-v8/source-freeze/source-<final-sha256>.json",sealedDayReceipts:["/srv/polyedge-ring/migration/qset-v8-seal/2026-09-09.json","/srv/polyedge-ring/migration/qset-v8-seal/2026-09-10.json"]},
  ...
  and (.protectedTrustRules.shadowQsetV8Processor | contains("no funded, qset-v1/v2/v3/v4/v5/v6/v7, Key Vault, Service Bus"))
' "$mapping" >/dev/null
"$root/test/test-qset-v8-processor-handoff.sh"
