...
handoff=$repo/ops/conduit/bin/polyedge-qset-v7-rbac-handoff
test_root=$(mktemp -d)
...
make_campaign() {
  ...
  case "$version" in 5) date=2026-08-26 ;; 6) date=2026-09-01 ;; 7) date=2026-09-02 ;; esac
  ...
  if test "$version" = 5; then sed -i 's#/assign/writer-#/assign/v5-writer-#g;s#/assign/processor-#/assign/v5-processor-#g' "$output_writer" "$output_processor"; elif test "$version" = 6; then sed -i 's#/assign/writer-#/assign/v6-writer-#g;s#/assign/processor-#/assign/v6-processor-#g' "$output_writer" "$output_processor"; fi
}
...
#!/usr/bin/env bash
case "$*" in
  'container exists polyedge-shadow-qset-v6') exit 1 ;;
...
case "$args" in
  ...
  'identity show'*)
    name=$(arg_after --name "$@")
    case "$name" in
      id-polyedge-conduit-shadow-qset-v7-writer) pid=writer-pid; cid=writer-client ;;
      id-polyedge-conduit-shadow-qset-v7-processor) pid=processor-pid; cid=processor-client ;;
      ...
      id-polyedge-conduit-shadow-qset-v6-writer) pid=v6-writer-pid; cid=v6-writer-client ;;
      id-polyedge-conduit-shadow-qset-v6-processor) pid=v6-processor-pid; cid=v6-processor-client ;;
  ...
  'identity federated-credential list'*)
    ...
    lane=${identity##*-v7-}; subject=spiffe://polyedge.local/conduit/shadow-qset-v7-$lane
    jq -nc --arg name "fic-spire-conduit-shadow-qset-v7-$lane" --arg subject "$subject" '[{name:$name,issuer:"https://oidc.jupiterlabs.dev",subject:$subject,audiences:["api://AzureADTokenExchange"]}]' |
...
case "$url" in
  ...
  *polyedge-shadow-qset-v7-events/shadow-events/preflight/*)
    if test "${DENY_POSITIVE:-}" = writer-preflight; then code=403; else code=201; fi ;;
  *polyedge-shadow-qset-v7-events/shadow-events/*) code=201 ;;
  *polyedge-research-qset-v7/data/research/*)
  ...
  *polyedge-shadow-qset-v7-events\?*|*polyedge-qset-v7-control\?*) code=200; body='<EnumerationResults />' ;;
...
setup_case() {
  ...
  make_campaign 6 v6-writer-pid v6-processor-pid "$STATE/assign-v6-writer-pid.json" "$STATE/assign-v6-processor-pid.json"
  make_campaign 7 writer-pid processor-pid "$STATE/full-writer.json" "$STATE/full-processor.json"
  jq -n --arg scope "$storage/blobServices/default/containers/polyedge-research-qset-v7" --arg role "$blob_reader" '[{id:"/assign/api-research",scope:$scope,roleDefinitionId:$role,principalId:"api-pid",condition:null,conditionVersion:null}]' >"$STATE/full-api.json"
...
run_handoff() {
  env PATH="$FAKE:$PATH" AZURE_RESOURCE_GROUP=rg AZURE_STORAGE_ACCOUNT_NAME=acct AZURE_TENANT_ID=tenant \
    QSET_V7_RBAC_RECEIPT_ROOT_TEST_ONLY="$RECEIPTS" QSET_V7_RBAC_WRITER_TOKEN_TEST_ONLY="$WRITER_TOKEN" QSET_V7_RBAC_PROCESSOR_TOKEN_TEST_ONLY="$PROCESSOR_TOKEN" \
...
casefold_live_resource_ids "$STATE/assign-v5-writer-pid.json" "$STATE/assign-v5-processor-pid.json" "$STATE/assign-v6-writer-pid.json" "$STATE/assign-v6-processor-pid.json" \
  "$STATE/full-writer.json" "$STATE/full-processor.json" "$STATE/full-api.json"
run_handoff check >/dev/null
run_handoff apply >/dev/null
jq -e '.schema=="polyedge.qset_v7_rbac_apply.v2" and .writerAssignments==5 and .processorAssignments==3 and .apiReaderAssignments==1' "$RECEIPTS/apply-result.json" >/dev/null
jq '(.writer[].id,.processor[].id,.apiResearchReader[].id) |= ascii_upcase' "$RECEIPTS/v7-assignments.json" >"$RECEIPTS/v7-assignments.json.tmp"
mv "$RECEIPTS/v7-assignments.json.tmp" "$RECEIPTS/v7-assignments.json"
...
cp "$STATE/full-api.json" "$STATE/assign-api-pid.json"
jq -n '{schema:"polyedge.qset_v7_rbac_before.v1",writerPrincipal:"writer-pid",processorPrincipal:"processor-pid",v7PrincipalsHaveZeroAssignments:true,apiV7ScopeHasZeroAssignments:true}' >"$RECEIPTS/before.json"; chmod 640 "$RECEIPTS/before.json"
jq -n --slurpfile writer "$STATE/full-writer.json" --slurpfile processor "$STATE/full-processor.json" --slurpfile api "$STATE/full-api.json" '{schema:"polyedge.qset_v7_exact_assignments.v1",writer:$writer[0],processor:$processor[0],apiResearchReader:$api[0]}' >"$RECEIPTS/v7-assignments.json"; chmod 640 "$RECEIPTS/v7-assignments.json"
run_handoff rollback >/dev/null
test "$(wc -l <"$STATE/delete.log")" = 4
jq -se 'all(.[];length==0)' "$STATE/assign-writer-pid.json" "$STATE/assign-processor-pid.json" "$STATE/assign-api-pid.json" >/dev/null
jq -e '.schema=="polyedge.qset_v7_rbac_rollback.v2" and .exactAssignmentsRemovedThisRun==4 and .exactV7AndApiAssignmentsAbsent==9' "$RECEIPTS/rollback-result.json" >/dev/null
jq -e 'length==5' "$STATE/assign-v5-writer-pid.json" >/dev/null; jq -e 'length==3' "$STATE/assign-v5-processor-pid.json" >/dev/null
jq -e 'length==5' "$STATE/assign-v6-writer-pid.json" >/dev/null; jq -e 'length==3' "$STATE/assign-v6-processor-pid.json" >/dev/null
run_handoff rollback >/dev/null
...
jq -se 'all(.[];length==0)' "$STATE/assign-writer-pid.json" "$STATE/assign-processor-pid.json" "$STATE/assign-api-pid.json" >/dev/null
test ! -e "$RECEIPTS/v7-assignments.json"
...
run_handoff verify-live | jq -e '
  .schema=="polyedge.qset_v7_rbac_verify_live.v1"
  and .writerAssignments==5 and .processorAssignments==3 and .apiReaderAssignments==1
  and (.v1ThroughV6FundedKeyVaultAndServiceBusDenied|length==2)
  and ([.v1ThroughV6FundedKeyVaultAndServiceBusDenied[].lane]|sort==["processor","writer"])
  and all(.v1ThroughV6FundedKeyVaultAndServiceBusDenied[]; .v1ThroughV6AndFundedStorageDenied and .keyVaultDenied and .serviceBusDenied)
' >/dev/null
jq '[.[0]]' "$STATE/full-writer.json" >"$STATE/assign-writer-pid.json"
...
printf '%s\n' 'qset-v7 RBAC proof tests passed'
