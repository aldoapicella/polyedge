...
handoff=$repo/ops/conduit/bin/polyedge-qset-v5-rbac-handoff
test_root=$(mktemp -d)
...
make_campaign() {
  ...
  if test "$version" = 3; then sed -i 's#/assign/writer-#/assign/v3-writer-#g;s#/assign/processor-#/assign/v3-processor-#g' "$output_writer" "$output_processor"; fi
}
...
#!/usr/bin/env bash
case "$1" in
  is-active) case "${@: -1}" in polyedge-shadow-qset-v3.service) exit 0 ;; *) exit 3 ;; esac ;;
  is-enabled) case "$2" in polyedge-shadow-qset-v3.service) echo enabled ;; *) echo disabled ;; esac ;;
  show) case "$*" in *MainPID*) echo 4242 ;; *) echo invocation-v3 ;; esac ;;
esac
STUB
...
case "$args" in
  ...
  'identity show'*)
    name=$(arg_after --name "$@")
    case "$name" in
      id-polyedge-conduit-shadow-qset-v5-writer) pid=writer-pid; cid=writer-client ;;
      id-polyedge-conduit-shadow-qset-v5-processor) pid=processor-pid; cid=processor-client ;;
      id-polyedge-conduit-shadow-qset-v3-writer) pid=v3-writer-pid; cid=v3-writer-client ;;
      id-polyedge-conduit-shadow-qset-v3-processor) pid=v3-processor-pid; cid=v3-processor-client ;;
  ...
  'identity federated-credential list'*)
    ...
    lane=${identity##*-v5-}; subject=spiffe://polyedge.local/conduit/shadow-qset-v5-$lane
    jq -nc --arg name "fic-spire-conduit-shadow-qset-v5-$lane" --arg subject "$subject" '[{name:$name,issuer:"https://oidc.jupiterlabs.dev",subject:$subject,audiences:["api://AzureADTokenExchange"]}]' |
...
case "$url" in
  ...
  *polyedge-shadow-qset-v5-events/shadow-events/preflight/*)
    if test "${DENY_POSITIVE:-}" = writer-preflight; then code=403; else code=201; fi ;;
  *polyedge-shadow-qset-v5-events/shadow-events/*) code=201 ;;
  *polyedge-research-qset-v5/data/research/*)
  ...
  *polyedge-shadow-qset-v5-events\?*|*polyedge-qset-v5-control\?*) code=200; body='<EnumerationResults />' ;;
...
run_handoff() {
  env PATH="$FAKE:$PATH" AZURE_RESOURCE_GROUP=rg AZURE_STORAGE_ACCOUNT_NAME=acct AZURE_TENANT_ID=tenant \
    QSET_V5_RBAC_RECEIPT_ROOT_TEST_ONLY="$RECEIPTS" QSET_V5_RBAC_WRITER_TOKEN_TEST_ONLY="$WRITER_TOKEN" QSET_V5_RBAC_PROCESSOR_TOKEN_TEST_ONLY="$PROCESSOR_TOKEN" \
...
jq -e '.schema=="polyedge.qset_v5_rbac_apply.v2" and .writerAssignments==5 and .processorAssignments==3 and .apiReaderAssignments==1' "$RECEIPTS/apply-result.json" >/dev/null
jq '(.writer[].id,.processor[].id,.apiResearchReader[].id) |= ascii_upcase' "$RECEIPTS/v5-assignments.json" >"$RECEIPTS/v5-assignments.json.tmp"
mv "$RECEIPTS/v5-assignments.json.tmp" "$RECEIPTS/v5-assignments.json"
...
cp "$STATE/full-api.json" "$STATE/assign-api-pid.json"
jq -n '{schema:"polyedge.qset_v5_rbac_before.v1",writerPrincipal:"writer-pid",processorPrincipal:"processor-pid",v5PrincipalsHaveZeroAssignments:true,apiV5ScopeHasZeroAssignments:true}' >"$RECEIPTS/before.json"; chmod 640 "$RECEIPTS/before.json"
jq -n --slurpfile writer "$STATE/full-writer.json" --slurpfile processor "$STATE/full-processor.json" --slurpfile api "$STATE/full-api.json" '{schema:"polyedge.qset_v5_exact_assignments.v1",writer:$writer[0],processor:$processor[0],apiResearchReader:$api[0]}' >"$RECEIPTS/v5-assignments.json"; chmod 640 "$RECEIPTS/v5-assignments.json"
run_handoff rollback >/dev/null
test "$(wc -l <"$STATE/delete.log")" = 4
jq -se 'all(.[];length==0)' "$STATE/assign-writer-pid.json" "$STATE/assign-processor-pid.json" "$STATE/assign-api-pid.json" >/dev/null
jq -e '.schema=="polyedge.qset_v5_rbac_rollback.v2" and .exactAssignmentsRemovedThisRun==4 and .exactV5AndApiAssignmentsAbsent==9' "$RECEIPTS/rollback-result.json" >/dev/null
jq -e 'length==5' "$STATE/assign-v3-writer-pid.json" >/dev/null; jq -e 'length==3' "$STATE/assign-v3-processor-pid.json" >/dev/null
run_handoff rollback >/dev/null
...
jq -se 'all(.[];length==0)' "$STATE/assign-writer-pid.json" "$STATE/assign-processor-pid.json" "$STATE/assign-api-pid.json" >/dev/null
test ! -e "$RECEIPTS/v5-assignments.json"
...
run_handoff verify-live | jq -e '
  .schema=="polyedge.qset_v5_rbac_verify_live.v1"
...
printf '%s\n' 'qset-v5 RBAC proof tests passed'
