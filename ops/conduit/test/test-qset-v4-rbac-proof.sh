...
handoff=$repo/ops/conduit/bin/polyedge-qset-v4-rbac-handoff
test_root=$(mktemp -d)
...
make_campaign() {
  ...
  case "$version" in 3) date=2026-08-23 ;; 4) date=2026-08-24 ;; esac
...
case "$args" in
  ...
  'identity show'*)
    name=$(arg_after --name "$@")
    case "$name" in
      id-polyedge-conduit-shadow-qset-v4-writer) pid=writer-pid; cid=writer-client ;;
      id-polyedge-conduit-shadow-qset-v4-processor) pid=processor-pid; cid=processor-client ;;
  ...
  'identity federated-credential list'*)
    ...
    lane=${identity##*-v4-}; subject=spiffe://polyedge.local/conduit/shadow-qset-v4-$lane
    jq -nc --arg name "fic-spire-conduit-shadow-qset-v4-$lane" --arg subject "$subject" '[{name:$name,issuer:"https://oidc.jupiterlabs.dev",subject:$subject,audiences:["api://AzureADTokenExchange"]}]' |
...
case "$url" in
  ...
  *polyedge-shadow-qset-v4-events/shadow-events/preflight/*)
...
setup_case() {
  ...
  jq -n --arg scope "$storage/blobServices/default/containers/polyedge-research-qset-v4" --arg role "$blob_reader" '[{id:"/assign/api-research",scope:$scope,roleDefinitionId:$role,principalId:"api-pid",condition:null,conditionVersion:null}]' >"$STATE/full-api.json"
...
cp "$STATE/full-api.json" "$STATE/assign-api-pid.json"
jq -n '{schema:"polyedge.qset_v4_rbac_before.v1",writerPrincipal:"writer-pid",processorPrincipal:"processor-pid",v4PrincipalsHaveZeroAssignments:true,apiV4ScopeHasZeroAssignments:true}' >"$RECEIPTS/before.json"; chmod 640 "$RECEIPTS/before.json"
jq -n --slurpfile writer "$STATE/full-writer.json" --slurpfile processor "$STATE/full-processor.json" --slurpfile api "$STATE/full-api.json" '{schema:"polyedge.qset_v4_exact_assignments.v1",writer:$writer[0],processor:$processor[0],apiResearchReader:$api[0]}' >"$RECEIPTS/v4-assignments.json"; chmod 640 "$RECEIPTS/v4-assignments.json"
...
jq -e '.schema=="polyedge.qset_v4_rbac_rollback.v2" and .exactAssignmentsRemovedThisRun==4 and .exactV4AndApiAssignmentsAbsent==9' "$RECEIPTS/rollback-result.json" >/dev/null
...
printf '%s\n' 'qset-v4 RBAC proof tests passed'
