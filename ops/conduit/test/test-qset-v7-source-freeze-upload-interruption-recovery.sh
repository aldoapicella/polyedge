...
freeze=$tmp/polyedge-qset-v7-source-freeze
sed "s#receipt_root=/srv/polyedge-ring/migration/qset-v7/source-freeze#receipt_root=$receipt_root#" "$root/bin/polyedge-qset-v7-source-freeze" >"$freeze"
chmod 0755 "$freeze"
...
jq -n --arg commit "$commit" --arg tree "$tree" --arg image "$image" '{
  ...
  evidence_version:"protocol-v3-qset-v7",
...
case "$*" in
  ...
  *"show HEAD:research/configs/campaign_freeze_2026-09-02_qset_v7.json")
...
! find "$tmp" -maxdepth 1 -type f \( -name 'tmp.*' -o -name '.qset-v7-freeze.*' \) | grep -q .
