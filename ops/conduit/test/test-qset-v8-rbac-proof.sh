...
handoff=$repo/ops/conduit/bin/polyedge-qset-v8-rbac-handoff
test_root=$(mktemp -d)
...
make_campaign() {
  ...
  case "$version" in 5) date=2026-08-26 ;; 6) date=2026-09-01 ;; 8) date=2026-09-09 ;; esac
...
#!/usr/bin/env bash
case "$*" in
  'container exists polyedge-shadow-qset-v5'|'container exists polyedge-shadow-qset-v6') exit 1 ;;
...
case "$args" in
  ...
  'identity show'*)
    name=$(arg_after --name "$@")
    case "$name" in
      id-polyedge-conduit-shadow-qset-v8-writer) pid=writer-pid; cid=writer-client ;;
      id-polyedge-conduit-shadow-qset-v8-processor) pid=processor-pid; cid=processor-client ;;
  ...
  'identity federated-credential list'*)
    ...
    lane=${identity##*-v8-}; subject=spiffe://polyedge.local/conduit/shadow-qset-v8-$lane
...
case "$url" in
  ...
  *polyedge-shadow-qset-v8-events/shadow-events/preflight/*)
    if test "${DENY_POSITIVE:-}" = writer-preflight; then code=403; else code=201; fi ;;
  *polyedge-shadow-qset-v8-events/shadow-events/*) code=201 ;;
  *polyedge-research-qset-v8/data/research/*)
  ...
  *polyedge-shadow-qset-v8-events\?*|*polyedge-qset-v8-control\?*) code=200; body='<EnumerationResults />' ;;
...
setup_case() {
  ...
  make_campaign 8 writer-pid processor-pid "$STATE/full-writer.json" "$STATE/full-processor.json"
  jq -n --arg scope "$storage/blobServices/default/containers/polyedge-research-qset-v8" --arg role "$blob_reader" '[{id:"/assign/api-research",scope:$scope,roleDefinitionId:$role,principalId:"api-pid",condition:null,conditionVersion:null}]' >"$STATE/full-api.json"
...
jq -e '.schema=="polyedge.qset_v8_rbac_apply.v2" and .writerAssignments==5 and .processorAssignments==3 and .apiReaderAssignments==1' "$RECEIPTS/apply-result.json" >/dev/null
jq '(.writer[].id,.processor[].id,.apiResearchReader[].id) |= ascii_upcase' "$RECEIPTS/v8-assignments.json" >"$RECEIPTS/v8-assignments.json.tmp"
mv "$RECEIPTS/v8-assignments.json.tmp" "$RECEIPTS/v8-assignments.json"
...
cp "$STATE/full-api.json" "$STATE/assign-api-pid.json"
jq -n '{schema:"polyedge.qset_v8_rbac_before.v1",writerPrincipal:"writer-pid",processorPrincipal:"processor-pid",v8PrincipalsHaveZeroAssignments:true,apiV8ScopeHasZeroAssignments:true}' >"$RECEIPTS/before.json"; chmod 640 "$RECEIPTS/before.json"
jq -n --slurpfile writer "$STATE/full-writer.json" --slurpfile processor "$STATE/full-processor.json" --slurpfile api "$STATE/full-api.json" '{schema:"polyedge.qset_v8_exact_assignments.v1",writer:$writer[0],processor:$processor[0],apiResearchReader:$api[0]}' >"$RECEIPTS/v8-assignments.json"; chmod 640 "$RECEIPTS/v8-assignments.json"
run_handoff rollback >/dev/null
test "$(wc -l <"$STATE/delete.log")" = 4
jq -se 'all(.[];length==0)' "$STATE/assign-writer-pid.json" "$STATE/assign-processor-pid.json" "$STATE/assign-api-pid.json" >/dev/null
jq -e '.schema=="polyedge.qset_v8_rbac_rollback.v2" and .exactV8AndApiAssignmentsRemovedThisRun==4 and .exactV8AndApiAssignmentsAbsent==9' "$RECEIPTS/rollback-result.json" >/dev/null
...
jq -se 'all(.[];length==0)' "$STATE/assign-writer-pid.json" "$STATE/assign-processor-pid.json" "$STATE/assign-api-pid.json" >/dev/null
test ! -e "$RECEIPTS/v8-assignments.json"
...
printf '%s\n' 'qset-v8 RBAC proof tests passed'
