...
helper=$root/bin/polyedge-qset-v7-retire-writer

run_helper() {
  source "$helper"
  receipt_root=$QSET_V7_RETIRE_TEST_ROOT/receipts
  receipt=$receipt_root/campaign-2026-09-02-qset-v7-writer.json
  lock_file=$QSET_V7_RETIRE_TEST_ROOT/retirement.lock
  wait_seconds=2

  systemctl() {
    case "$1" in
      is-active) test -e "$QSET_V7_RETIRE_TEST_ROOT/active" ;;
      show)
        case "$*" in *InvocationID*) cat "$QSET_V7_RETIRE_TEST_ROOT/invocation" ;; *MainPID*) echo 4321 ;; *) return 2 ;; esac ;;
      stop)
        echo stop >>"$QSET_V7_RETIRE_TEST_ROOT/actions"
        if test ! -e "$QSET_V7_RETIRE_TEST_ROOT/stop-failed"; then touch "$QSET_V7_RETIRE_TEST_ROOT/stop-failed"; return 75; fi
        rm -f "$QSET_V7_RETIRE_TEST_ROOT/active" ;;
      *) return 2 ;;
    esac
  }
  podman() {
    case "$1" in
      inspect)
        jq -nc --arg id "$(cat "$QSET_V7_RETIRE_TEST_ROOT/container")" --arg image "$(cat "$QSET_V7_RETIRE_TEST_ROOT/image")" --arg digest "$(cat "$QSET_V7_RETIRE_TEST_ROOT/digest")" '[{Id:$id,Config:{Image:$image},ImageDigest:$digest,State:{Status:"running"}}]' ;;
      image) cat "$QSET_V7_RETIRE_TEST_ROOT/revision" ;;
      kill) echo USR1 >>"$QSET_V7_RETIRE_TEST_ROOT/actions" ;;
      *) return 2 ;;
    esac
  }
  journalctl() {
...
export QSET_V7_RETIRE_TEST_ROOT=$test_root
...
jq -nc --arg digest "$digest" --arg revision "$revision" '{schema:"polyedge.qset_v7_writer_retirement_receipt.v1",status:"prepared_for_retirement",retired_at:"2026-08-22T10:00:00Z",campaign_id:"campaign-2026-09-02-qset-v7",app_name:"polyedge-shadow-qset-v7",image_digest:$digest,source_revision:$revision,recorder_instance_id:"11111111-2222-4333-8444-555555555555",final_assigned_sequence:9,final_enqueued_sequence:9,final_enqueued_total:9,final_persisted_sequence:9,final_persisted_total:9,final_queued:0,flush_success:true}' >"$test_root/journal-message"

if fake_root; then echo 'first stop failure was accepted' >&2; exit 1; fi
evidence=$test_root/receipts/campaign-2026-09-02-qset-v7-writer.json
test -s "$evidence"
