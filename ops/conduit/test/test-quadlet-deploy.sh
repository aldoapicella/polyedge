...
printf '[Container]\nImage=%s\nContainerName=polyedge-shadow-qset-v8\n' "$old" >"$tmp/quadlets/polyedge-shadow-qset-v8.container"
...
run "$next" "$next" 20260805T000004Z polyedge-shadow-qset-v8
grep -Fx "Image=$next" "$tmp/quadlets/polyedge-shadow-qset-v8.container" >/dev/null
...
if run "$next" "$next" 20260805T000006Z polyedge-shadow-qset-v7; then
  echo 'retired qset deployment was accepted' >&2
  exit 1
fi
...
grep -Fx 'FUNDED_DIRECT_SERVICE_BUS_ENABLED=false' "$root/env/funded-intent-producer.env.example" >/dev/null
grep -Fx 'FUNDED_DIRECT_SERVICE_BUS_NAMESPACE=' "$root/env/funded-intent-producer.env.example" >/dev/null
grep -Fx 'FUNDED_DIRECT_SERVICE_BUS_QUEUE=' "$root/env/funded-intent-producer.env.example" >/dev/null
grep -Fx 'FUNDED_DIRECT_OCI_QUEUE_BRIDGE_URL=http://10.89.0.1:8182/v1/messages' "$root/env/funded-intent-producer.env.example" >/dev/null
