...
write_env() {
  cat >"$env_file" <<'EOF'
UNCHANGED=before
POLYEDGE_QSET_V8_DAY1_RECEIPT_SHA256=
POLYEDGE_QSET_V8_DAY1_INVENTORY_SHA256=
POLYEDGE_QSET_V8_DAY2_RECEIPT_SHA256=
POLYEDGE_QSET_V8_DAY2_INVENTORY_SHA256=
EOF
  chmod 0640 "$env_file"
}
...
setup_case() {
  ...
  env_file=$case_dir/qset-v8-processor.env
  gate=$case_dir/ENABLE_QSET_V8_PROCESSOR_MANUAL
  ...
  : >"$state/order"
  for date in 2026-09-09 2026-09-10; do
    day=${date##*-}; day=${day#0}; digest=$(printf '%064d' "$day")
    printf '{"schema":"polyedge.qset_v8_closed_day_seal.v1","campaign_id":"campaign-2026-09-09-qset-v8","date":"%s","source_inventory_sha256":"sha256:%s"}\n' "$date" "$digest" >"$receipts/$date.json"
...
test "${POLYEDGE_QSET_V8_DAY1_RECEIPT_SHA256#sha256:}" != "$POLYEDGE_QSET_V8_DAY1_RECEIPT_SHA256"
EOF
  cat >"$disk_guard" <<'EOF'
#!/bin/sh
...
case "$1" in
    ...
    case "$2" in
      ...
      --property=Result)
        if test -f "$state/inject-second-start"; then
          ...
          if "$0" start --no-block polyedge-qset-v8-processor.service >/dev/null 2>&1; then exit 70; fi
  ...
  start)
    test "$2" = --no-block
    test "$3" = polyedge-qset-v8-processor.service
...
run_handoff() {
  QSET_V8_PROCESSOR_HANDOFF_TEST_ONLY=1 \
  QSET_V8_PROCESSOR_HANDOFF_ENV_FILE_TEST_ONLY="$env_file" \
  QSET_V8_PROCESSOR_HANDOFF_RECEIPT_ROOT_TEST_ONLY="$receipts" \
  QSET_V8_PROCESSOR_HANDOFF_GATE_FILE_TEST_ONLY="$gate" \
  QSET_V8_PROCESSOR_HANDOFF_PREFLIGHT_TEST_ONLY="$preflight" \
  QSET_V8_PROCESSOR_HANDOFF_DISK_GUARD_TEST_ONLY="$disk_guard" \
  QSET_V8_PROCESSOR_HANDOFF_SYSTEMCTL_TEST_ONLY="$systemctl" \
  QSET_V8_PROCESSOR_HANDOFF_RBAC_CHECKER_TEST_ONLY="$rbac_checker" \
  QSET_V8_PROCESSOR_HANDOFF_LOCK_FILE_TEST_ONLY="$lock" \
  QSET_V8_PROCESSOR_HANDOFF_ROLLBACK_ROOT_TEST_ONLY="$rollback" \
  QSET_V8_PROCESSOR_HANDOFF_EXPECTED_UID_TEST_ONLY="$uid" \
  QSET_V8_PROCESSOR_HANDOFF_EXPECTED_GID_TEST_ONLY="$gid" \
  QSET_V8_PROCESSOR_HANDOFF_START_ATTEMPTS_TEST_ONLY=5 \
  QSET_V8_PROCESSOR_HANDOFF_WAIT_ATTEMPTS_TEST_ONLY=5 \
...
jq -e '.schema=="polyedge.qset_v8_processor_attempt.v2"' "$rollback/attempt.json" >/dev/null
printf '%032d\n' 0 >"$state/invocation"
run_handoff >/dev/null
test "$(cat "$state/starts")" = 1
...
QSET_V8_PROCESSOR_HANDOFF_CRASH_AFTER_ATTEMPT_TEST_ONLY=1
export QSET_V8_PROCESSOR_HANDOFF_CRASH_AFTER_ATTEMPT_TEST_ONLY
if run_handoff >/dev/null 2>&1; then echo 'crash-after-attempt injection was accepted' >&2; exit 1; fi
unset QSET_V8_PROCESSOR_HANDOFF_CRASH_AFTER_ATTEMPT_TEST_ONLY
test -f "$rollback/attempt.json"
test -f "$rollback/qset-v8-processor.env.before"
test ! -e "$rollback/started.json"
test ! -e "$gate"
test "$(cat "$state/starts")" = 0
cmp -s "$env_file" "$rollback/qset-v8-processor.env.before"
run_handoff >/dev/null
test "$(cat "$state/starts")" = 1
test -f "$rollback/completed.json"

setup_case exit-after-env
QSET_V8_PROCESSOR_HANDOFF_EXIT_AFTER_ENV_TEST_ONLY=1
export QSET_V8_PROCESSOR_HANDOFF_EXIT_AFTER_ENV_TEST_ONLY
if run_handoff >/dev/null 2>&1; then echo 'exit-after-env injection was accepted' >&2; exit 1; fi
unset QSET_V8_PROCESSOR_HANDOFF_EXIT_AFTER_ENV_TEST_ONLY
test -f "$rollback/attempt.json"
test ! -e "$rollback/started.json"
test ! -e "$gate"
test "$(cat "$state/starts")" = 0
...
unset QSET_V8_PROCESSOR_HANDOFF_CRASH_AFTER_ATTEMPT_TEST_ONLY
chmod 0600 "$rollback/qset-v8-processor.env.before"
...
printf '%s\n' 'qset-v8 processor handoff tests passed'
