...
from analyze_24h_report import (
    included_hour_prefixes,
    market_summary,
    max_drawdown,
    render_markdown,
    required_detect_mean,
    required_precision,
    sample_std,
)
...
def build_payload(
    ...
    return {
        ...
        "bugs_or_risks": bug_risk_notes(selection),
...
def enrich_time_buckets(
    ...
    for fill in collector.runtime_fills:
        ...
        collector.time_buckets[bucket]["runtime_fills"] += 1
...
def bug_risk_notes(selection: BlobSelection) -> list[dict[str, Any]]:
    return [
        {
            "severity": "P1",
            "status": "fixed",
            "area": "CLOB market feed rollover",
            "finding": (
                "The market feed subscribed to the token set once and did not resubscribe when new "
                "15-minute markets rolled in, causing current-market missing-book holds."
            ),
            "evidence": "Patched src/polyedge/bot.py and verified post-deploy book events resumed.",
        },
        {
            "severity": "P1",
            "status": "fixed",
            ...
            "evidence": "Patched src/polyedge/backtest.py and added regression tests.",
        },
        {
            "severity": "P2",
            "status": "observed",
            ...
            "finding": (
                "The 24h book stream is multi-GiB. Long synchronous /pnl requests remain inappropriate; "
                "use background reports or this streaming analyzer."
            ),
            "evidence": (
                f"Rolling 24h selection contains {len(selection.blobs)} blobs and "
                f"about {selection.total_bytes / 1024**3:.3f} GiB."
            ),
        },
    ]
