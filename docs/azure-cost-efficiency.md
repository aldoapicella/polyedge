...
| Snapshot storage | A lifecycle rule starts deletion only for `bot-events/data/research/normalized/v1/` block blobs after seven days; the existing 14-day soft-delete recovery window still applies. | Raw events, daily reports, replay outputs, qset evidence, and funded evidence are outside the prefix and are retained. |
