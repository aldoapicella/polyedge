...
| Storage lifecycle | One rule deletes only `bot-events/data/research/normalized/v1/` block blobs after seven days. A second tiers only future `bot-events/events-oci-hot7-v1/` block blobs from Hot to Cool after seven days; the existing 14-day soft-delete recovery window still applies. | The existing raw corpus, daily reports, replay outputs, qset evidence, and funded evidence are outside the new prefix and are retained. |
