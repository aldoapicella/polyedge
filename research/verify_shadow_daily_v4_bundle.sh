...
test "${SHADOW_CAMPAIGN_ID:?SHADOW_CAMPAIGN_ID is required}" = "campaign-2026-08-24-qset-v4" \
  ...
  && test "${SHADOW_CODE_FREEZE_FINALIZED:-false}" = "true" || {
    echo "qset-v4 bundle verifier binding is inexact or draft" >&2
    exit 1
  }
...
case "${EXECUTION_FREEZE_ARTIFACT_PATH:?EXECUTION_FREEZE_ARTIFACT_PATH is required}" in
  reports/research/shadow/campaigns/campaign-2026-08-24-qset-v4/control/code-freeze/source-"$FREEZE_DIGEST".json) ;;
  *) echo "qset-v4 source freeze path is outside its control boundary" >&2; exit 1 ;;
esac
case "${SHADOW_CODE_FREEZE_MANIFEST:?SHADOW_CODE_FREEZE_MANIFEST is required}" in
  azure://"${AZURE_STORAGE_ACCOUNT_NAME:?AZURE_STORAGE_ACCOUNT_NAME is required}"/polyedge-qset-v4-control/"$EXECUTION_FREEZE_ARTIFACT_PATH") ;;
  *) echo "qset-v4 source freeze manifest is outside its control boundary" >&2; exit 1 ;;
esac
