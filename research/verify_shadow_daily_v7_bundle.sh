...
test "$#" -ge 1 || {
  echo "usage: verify_shadow_daily_v7_bundle.sh YYYY-MM-DD [YYYY-MM-DD ...]" >&2
  exit 2
}
validate_qset_v7_bundle_date() {
  value=$1
  case "$value" in 2026-09-0[1-9]|2026-09-[12][0-9]|2026-09-30|2026-10-0[1-9]|2026-10-[12][0-9]|2026-10-31) ;; *) echo "qset-v7 verifier rejects date outside 2026-09-02 through 2026-10-31: $value" >&2; exit 2 ;; esac
}
for requested_date in "$@"; do validate_qset_v7_bundle_date "$requested_date"; done


test "${SHADOW_CAMPAIGN_ID:?SHADOW_CAMPAIGN_ID is required}" = "campaign-2026-09-02-qset-v7" \
  && test "${SHADOW_EVIDENCE_VERSION:?SHADOW_EVIDENCE_VERSION is required}" = "protocol-v3-qset-v7" \
  ...
  && test "${SHADOW_CODE_FREEZE_FINALIZED:-false}" = "true" || {
    echo "qset-v7 bundle verifier binding is inexact or draft" >&2
    exit 1
  }
...
case "${EXECUTION_FREEZE_ARTIFACT_PATH:?EXECUTION_FREEZE_ARTIFACT_PATH is required}" in
  reports/research/shadow/campaigns/campaign-2026-09-02-qset-v7/control/code-freeze/source-"$FREEZE_DIGEST".json) ;;
  *) echo "qset-v7 source freeze path is outside its control boundary" >&2; exit 1 ;;
esac
case "${SHADOW_CODE_FREEZE_MANIFEST:?SHADOW_CODE_FREEZE_MANIFEST is required}" in
  azure://"${AZURE_STORAGE_ACCOUNT_NAME:?AZURE_STORAGE_ACCOUNT_NAME is required}"/polyedge-qset-v7-control/"$EXECUTION_FREEZE_ARTIFACT_PATH") ;;
  *) echo "qset-v7 source freeze manifest is outside its control boundary" >&2; exit 1 ;;
esac
