...
test "$#" -ge 1 || {
  echo "usage: verify_shadow_daily_v8_bundle.sh YYYY-MM-DD [YYYY-MM-DD ...]" >&2
  exit 2
}
validate_qset_v8_bundle_date() {
  value=$1
  case "$value" in 2026-09-09|2026-09-1[0-9]|2026-09-2[0-9]|2026-09-30|2026-10-0[1-9]|2026-10-[12][0-9]|2026-10-3[01]|2026-11-0[1-7]) ;; *) echo "qset-v8 verifier rejects date outside 2026-09-09 through 2026-11-07: $value" >&2; exit 2 ;; esac
}
for requested_date in "$@"; do validate_qset_v8_bundle_date "$requested_date"; done


test "${SHADOW_CAMPAIGN_ID:?SHADOW_CAMPAIGN_ID is required}" = "campaign-2026-09-09-qset-v8" \
  && test "${SHADOW_EVIDENCE_VERSION:?SHADOW_EVIDENCE_VERSION is required}" = "protocol-v3-qset-v8" \
  ...
  && test "${SHADOW_CODE_FREEZE_FINALIZED:-false}" = "true" || {
    echo "qset-v8 bundle verifier binding is inexact or draft" >&2
    exit 1
  }
...
case "${EXECUTION_FREEZE_ARTIFACT_PATH:?EXECUTION_FREEZE_ARTIFACT_PATH is required}" in
  reports/research/shadow/campaigns/campaign-2026-09-09-qset-v8/control/code-freeze/source-"$FREEZE_DIGEST".json) ;;
  *) echo "qset-v8 source freeze path is outside its control boundary" >&2; exit 1 ;;
esac
case "${SHADOW_CODE_FREEZE_MANIFEST:?SHADOW_CODE_FREEZE_MANIFEST is required}" in
  azure://"${AZURE_STORAGE_ACCOUNT_NAME:?AZURE_STORAGE_ACCOUNT_NAME is required}"/polyedge-qset-v8-control/"$EXECUTION_FREEZE_ARTIFACT_PATH") ;;
  *) echo "qset-v8 source freeze manifest is outside its control boundary" >&2; exit 1 ;;
esac
